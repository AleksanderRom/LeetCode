#include <cassert>
#include <string>
#include <vector>
// #include <cmath>

using namespace std;

class Solution {
   public:
    int ChToInt(char ch) {
        return ch - 48;
    }

    void Inizializate(vector<vector<int>>& dp, string& num1, string& num2){
        int n1 = num1.size();
        int n2 = num2.size();
        for (int i = 0; i < n2; ++i) {
            for (int j = 0; j < n1; ++j) {
                auto in1 = ChToInt(num1[n1 - j - 1]);
                auto in2 = ChToInt(num2[n2 - i - 1]);
                dp[i][j] = in1 * in2;
            }
        }
    }

    string multiply(string num1, string num2) {
        if (num1[0] == '0' || num2[0] == '0') {
            return "0";
        }

        int n1 = num1.size();
        int n2 = num2.size();
        if (n2 > n1) {
            std::swap(num1, num2);
            std::swap(n1, n2);
        }
        vector<vector<int>> dp(n2, vector<int>(n1));
        Inizializate(dp, num1, num2);

        string result;
        for (int i = 0; i < n1 + n2 - 1; ++i) {
            int buf = 0;

            for (int j = 0, k = i; j < i + 1; ++j, --k) {
                if (j >= n2 || k >= n1) {
                    continue;
                }
                buf += dp[j][k];
            }

            result.push_back(GetCh(buf) + 48);
        }
        if (remains_ > 0) {
            result.push_back(remains_ + 48);
        }
        remains_ = 0;
        return string(result.rbegin(), result.rend());
    }

    char GetCh(int buf) {
        buf += remains_;
        char ch = 0;

        if (buf >= 10) {
            remains_ = buf / 10;
            return buf % 10;
        }
        remains_ = 0;
        return ch = buf;
    }

   private:
    int remains_ = 0;
};

void Test();

int main() {
    Test();
}

void Test() {
    Solution s;
    assert("492" == s.multiply("123", "4"));
    assert("0" == s.multiply("4158", "0"));
    auto ddd = s.multiply("123456789", "987654321");
    assert("121932631112635269"s == s.multiply("123456789", "987654321"));


    auto t = s.multiply("2", "3");
    assert("6" == s.multiply("2", "3"));
    assert("56088" == s.multiply("123", "456"));
    assert("492" == s.multiply("4", "123"));
    assert("0" == s.multiply("4", "0"));
    assert("0" == s.multiply("4158", "0"));
    assert("0" == s.multiply("0", "1586"));
    assert("998999999001" == s.multiply("999", "999999999"));
}